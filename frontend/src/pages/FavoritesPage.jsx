...
const FavoritesPage = () => {
  ...
  useEffect(() => {
    // Simulate getting favorites from mockData
    // For now, let's get 2 random items from listings as favorites
    const randomFavorites = listings
      .sort(() => 0.5 - Math.random())
      .slice(0, 2)
      .map(item => ({
        ...item,
        folderId: 2 // Assigning to Electronics folder for demo
      }));
        ...
        {filteredFavorites.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredFavorites.map(item => (
              <div 
                key={item.id} 
                className="bg-white rounded-lg shadow-md overflow-hidden group transform transition-all hover:shadow-lg cursor-pointer"
                ...
                <div className="relative overflow-hidden rounded-t-lg">
                  ...
                  <div className="absolute top-3 right-3 flex gap-2">
                    ...
                    <button 
                      ...
                      className="p-1.5 rounded-full bg-white shadow-md text-gray-600 hover:bg-gray-100 transition-colors"
                    >
